...
// // ── Permissions accordées par l'admin ─────────────────────────────────────────
// const CAN_ADD_PAYMENT = false;
// const CAN_VIEW_ALL_CANDIDATES = false; // false = voit uniquement ses propres candidats

// // Moniteur actuellement connecté (simulé)
...
//   // ── Chargement des données (uniquement les candidats du moniteur) ───────────
//   const fetchData = useCallback(async () => {
//     try {
//       const payments = await window.electron.getPayments();
//       // Filtrage côté front : uniquement les paiements liés à ce moniteur
//       // (idéalement ce filtre se fait côté backend/IPC)
//       const myPayments = CAN_VIEW_ALL_CANDIDATES
//         ? payments
//         : (payments || []).filter(p => p.moniteur === CURRENT_MONITEUR);

//       setPaymentsData(myPayments);

//       // Stats simplifiées pour le moniteur
...
//         ? Math.round((myPayments.filter(p => p.montantRestant === 0).length / myPayments.length) * 100)
//         : 0;

//       setStats({
//         total:     `${totalEncaisse} DA`,
//         enAttente: `${totalRestant} DA`,
//         taux:      `${taux}%`,
//       });
//     } catch (err) {
//       console.error("Erreur lors du chargement des paiements :", err);
//       // Données simulées si electron n'est pas dispo (développement web)
//       setPaymentsData([
//         { idVersement: 1, prenom: "Karima", nom: "Alhane",    dateVersement: "2026-03-10", montant: 8000,  montantRestant: 4000, methode: "espèces",  moniteur: "Moniteur 3" },
//         { idVersement: 2, prenom: "Bssad",  nom: "Omar",      dateVersement: "2026-03-12", montant: 12000, montantRestant: 0,    methode: "virement", moniteur: "Moniteur 3" },
//         { idVersement: 3, prenom: "Nassima",nom: "Oukili",    dateVersement: "2026-03-05", montant: 5000,  montantRestant: 7000, methode: "espèces",  moniteur: "Moniteur 3" },
//       ]);
...
//       return;
//     }
//     // logique réelle ici si permission accordée
//   };
...
//             Suivi des Paiements
//           </h1>
//           <p>Versements de mes candidats — {CURRENT_MONITEUR}</p>
//         </div>
...
//           {[
//             { title: "Encaissé (mes candidats)", value: stats.total,     color: "#2b537e", detail: "Total reçu" },
...
//           Vue lecture seule — contactez l'admin pour ajouter ou modifier des paiements
//         </div>
...
//                       {/* Pas de colonne Actions pour le moniteur en lecture seule */}
//                     </tr>
//                   ))
//                 ) : (
//                   <tr>
...
// ── Reçoit l'objet user depuis le parent (ex: { id: 3, nom: "Dupont", ... })
const PaiementsMoniteur = () => {
  const { currentUser } = useAuth();  // ← récupère depuis le contexte
  const moniteurId = currentUser?.id;  // ← id du moniteur connecté
  ...
  // ── Chargement des versements du moniteur ──────────────────────────────────
  const fetchData = useCallback(async () => {
    if (!moniteurId) return;
    try {
      const payments = await window.electron.getPaymentsByMoniteur(moniteurId);
  ...
  // ── Ajout de paiement (même logique que l'admin) ───────────────────────────
  ...
  // ── Stats dérivées ─────────────────────────────────────────────────────────
  const totalEncaisse  = paymentsData.reduce((acc, p) => acc + (p.montant || 0), 0);
  const totalRestant   = paymentsData.reduce((acc, p) => acc + (p.montantRestant || 0), 0);
  const tauxRecouvrement = paymentsData.length
    ? Math.round(
        (paymentsData.filter(p => p.montantRestant === 0).length / paymentsData.length) * 100
      )
    : 0;
    ...
    <div className="container">
      <div className="main">
        ...
        <div className="header">
          ...
          <p>Versements de mes candidats</p>
        </div>

        {/* STATS */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "20px", marginBottom: "30px", marginTop: "20px" }}>
          {[
            { title: "Encaissé (mes candidats)", value: `${totalEncaisse.toLocaleString("fr-DZ")} DA`, color: "#2b537e", detail: "Total reçu" },
            { title: "Reste à recouvrer",        value: `${totalRestant.toLocaleString("fr-DZ")} DA`,  color: "#011659", detail: "Soldes en attente" },
            { title: "Taux de recouvrement",     value: `${tauxRecouvrement}%`,                         color: "#166534", detail: "Dossiers soldés" },
          ].map((card, i) => (
            <div key={i} style={{
              background: "#DDE2EF", borderRadius: "16px", padding: "24px",
              borderLeft: `6px solid ${card.color}`, boxShadow: "0 2px 8px rgba(0,0,0,0.05)"
            }}>
              <p style={{ margin: 0, fontSize: "12px", fontWeight: "700", color: "#64748b", textTransform: "uppercase" }}>{card.title}</p>
              <h2 style={{ margin: "8px 0", fontSize: "26px", fontWeight: "800", color: "#1e293b" }}>{card.value}</h2>
              <p style={{ margin: 0, fontSize: "13px", color: card.color, fontWeight: "600" }}>{card.detail}</p>
