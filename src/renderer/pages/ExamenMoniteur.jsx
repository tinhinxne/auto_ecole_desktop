...
// ── Permission accordée par l'admin ──────────────────────────────────────────
// Mettre à true si l'admin a accordé la permission de modifier les résultats
...
// ── Page Examens (vue Moniteur) ────────────────────────────────────────────────
const ExamensMoniteur = () => {
  // Seules les sessions du moniteur connecté lui sont affichées
  ...
  const handleRemoveCandidat = (e) => {
    e.stopPropagation();
    if (!CAN_REMOVE_CANDIDAT) {
      setLockedBanner("La suppression d'un candidat est réservée aux administrateurs. Contactez votre admin.");
      return;
    }
  ...
  const handleToggleStatus = (e) => {
    ...
    if (!CAN_TOGGLE_STATUS) {
      setLockedBanner("La modification du résultat est réservée aux administrateurs. Contactez votre admin.");
      return;
    }
    // logique réelle ici si permission accordée
  };
  ...
  const filtered = examensList.filter((ex) => {
    ...
    const matchType   = typeFilter   === "Tous" || ex.type   === typeFilter;
        ...
        <div className="examens-page-header">
          <div>
            ...
            <p className="examens-page-sub">
              Vue lecture seule — contactez l'admin pour modifier les résultats.
            </p>
          </div>
                    ...
                    >
                      ...
                      <td>
                        <div className="examens-table__date">
                          ...
                          <div>
                            {examen.date}
                            <span className="examens-table__heure"> {examen.heure}</span>
                          </div>
                        </div>
                      </td>
                      <td>{examen.lieu}</td>

                      {/* Statut — verrouillé si pas de permission */}
                      <td>
                        {CAN_TOGGLE_STATUS ? (
                          <div
                          ...
                          <LockedTooltip message="Modification réservée à l'admin">
                            <div
                              ...
                              onClick={(e) => handleToggleStatus(e)}
                            >
                              <FaLock style={{ marginRight: 6, fontSize: 9, opacity: 0.6 }} />
                              {st.label}
                            </div>
                          </LockedTooltip>
                        )}
                      ...
                      <td className="examens-table__actions">
                        {CAN_REMOVE_CANDIDAT ? (
                          <button className="btn-remove" onClick={(e) => handleRemoveCandidat(e)}>
                            🗑
                          </button>
                        ) : (
                          <LockedTooltip message="Suppression réservée à l'admin">
                            <button
                              ...
                              style={{ opacity:0.6, cursor: "not-allowed",  filter: "grayscale(2) "}}
                              onClick={(e) => handleRemoveCandidat(e)}
                            >
                              🗑
                            </button>
                          </LockedTooltip>
